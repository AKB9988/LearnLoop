...
public class BountyBoardFragment extends Fragment implements BountyAdapter.OnBountyClickListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        swipeRefresh.setOnRefreshListener(this::loadActiveRequests);
                    ...
                    .enqueue(new Callback<ApiResponse<List<HelpRequest>>>() {
                        ...
                        public void onResponse(Call<ApiResponse<List<HelpRequest>>> call,
                            ...
                            if (response.isSuccessful() && response.body() != null && response.body().isSuccess()) {
                                ...
                                loadMockData(true); // Appends demo requests so board looks full

                            } else {
                                // Fallback to mock data if server returns error
                                String errorMsg = "API Error: " + response.code();
                                if (response.body() != null && !response.body().isSuccess()) {
                                    errorMsg = "API Error: isSuccess is false. Data mismatch?";
                                }
                        ...
                        public void onFailure(Call<ApiResponse<List<HelpRequest>>> call, Throwable t) {
                            ...
                            Toast.makeText(requireContext(), "Network Failure: " + t.getMessage(), Toast.LENGTH_LONG).show();
                            loadMockData();
                        }
                    });
    ...
    private TextView chipAll, chipForYou, chipHighPriority, chipRecent, chipMyRequests;
    ...
    private void bindChips(View view) {
        ...
        chipMyRequests = view.findViewById(R.id.chipMyRequests);
        ...
        View.OnClickListener chipListener = v -> {
            ...
            chipMyRequests.setBackgroundResource(R.drawable.bg_chip_unselected);
            ...
            } else if (v.getId() == R.id.chipMyRequests) {
                FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
                String myName = currentUser != null && currentUser.getDisplayName() != null ? currentUser.getDisplayName() : "Learner";
                for (HelpRequest req : allRequests) {
                    if (myName.equals(req.getPosterName())) requestList.add(req);
                }
            } else {
        ...
        chipMyRequests.setOnClickListener(chipListener);
    }
    ...
    private void loadMockData(boolean append) {
        if (!append) allRequests.clear();

        FirebaseUser currentUser = FirebaseAuth.getInstance().getCurrentUser();
        String myName = currentUser != null && currentUser.getDisplayName() != null ? currentUser.getDisplayName() : "Learner";

        allRequests.add(new HelpRequest(
                "mock_my_accepted", "Review my React project architecture",
                "I have built a web app. Need someone to quickly review the component structure.",
                "Web Dev", myName, null, 100, "medium", 1.0,
                "accepted", "2026-05-15", new String[]{"React", "JavaScript"},
                "video", "English", 45, 99, 0
        ));
    ...
    private void showLoading(boolean show) {
        if (swipeRefresh != null && swipeRefresh.isRefreshing()) {
            progressBounty.setVisibility(View.GONE);
        } else {
    ...
    public void onAcceptClicked(HelpRequest request) {
        ...
        // If I am the requester, just join the room directly! No need to call accept API.
        if (user.getDisplayName() != null && user.getDisplayName().equals(request.getPosterName())) {
            Toast.makeText(requireContext(), "Joining your session room...", Toast.LENGTH_SHORT).show();
            openSessionUrl(request);
            return;
        }
    ...
    private void openSessionUrl(HelpRequest request) {
        String sessionType = request.getSessionType() != null ? request.getSessionType().toLowerCase() : "video";
        ...
        String safeId = "DemoRoom2026";
        String sessionUrl;
        if (sessionType.contains("chat")) {
            sessionUrl = "https://tlk.io/learnloop" + safeId;
        } else if (sessionType.contains("board") || sessionType.contains("white")) {
